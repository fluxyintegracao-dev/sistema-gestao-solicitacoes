...
const CHAVE_SETORES_SEM_ALTERACAO_STATUS = 'SETORES_SEM_ALTERACAO_STATUS';
...
async function obterConfiguracaoSetoresSemAlteracaoStatus() {
  const item = await ConfiguracaoSistema.findOne({
    where: { chave: CHAVE_SETORES_SEM_ALTERACAO_STATUS },
    order: [['id', 'DESC']]
  });
...
async function salvarConfiguracaoSetoresSemAlteracaoStatus(setores) {
  const payload = {
  ...
  if (existente) {
    ...
    await ConfiguracaoSistema.create({
      chave: CHAVE_SETORES_SEM_ALTERACAO_STATUS,
      valor
    });
  }
...
async function obterTokensSetoresSemAlteracaoStatus() {
  const { setores } = await obterConfiguracaoSetoresSemAlteracaoStatus();
...
function setorEstaSemAlteracaoStatus(setor, tokensBloqueados = []) {
  const setorNormalizado = normalizarTokenSetor(setor);
  if (!setorNormalizado) return false;

  return (Array.isArray(tokensBloqueados) ? tokensBloqueados : [])
    .map(normalizarTokenSetor)
    .some((token) => tokensSaoEquivalentes(token, setorNormalizado));
}

module.exports = {
  CHAVE_SETORES_SEM_ALTERACAO_STATUS,
