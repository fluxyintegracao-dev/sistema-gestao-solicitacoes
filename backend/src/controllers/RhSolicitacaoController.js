...
async function obrasVisiveis(req) {
  ...
  const vinculos = Array.isArray(req.user?.obras) ? req.user.obras : [];
  return vinculos.map((obra) => Number(obra.id || obra)).filter(Boolean);
}
...
module.exports = {
  async index(req, res) {
    try {
      ...
      const obraIds = await obrasVisiveis(req);
  ...
  async create(req, res) {
    try {
      const criada = await abrirSolicitacao(req.body || {}, contextoDe(req));
