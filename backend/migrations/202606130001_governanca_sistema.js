...
module.exports = {
  async up({ DataTypes, queryInterface, sequelize }) {
    if (!(await tableExists(sequelize, 'governanca_snapshots'))) {
      await queryInterface.createTable('governanca_snapshots', {
        ...
        createdAt: { type: DataTypes.DATE, allowNull: false, defaultValue: queryInterface.sequelize.literal('CURRENT_TIMESTAMP') },
        updatedAt: { type: DataTypes.DATE, allowNull: false, defaultValue: queryInterface.sequelize.literal('CURRENT_TIMESTAMP') }
      });
    }

    if (!(await indexExists(sequelize, 'governanca_snapshots', 'uk_governanca_snapshots_data'))) {
    ...
    if (!(await tableExists(sequelize, 'governanca_access_logs'))) {
    ...
    if (!(await columnExists(sequelize, 'governanca_access_logs', 'usuario_id'))) {
    ...
    for (const [name, fields] of indexes) {
      if (!(await indexExists(sequelize, 'governanca_access_logs', name))) {
