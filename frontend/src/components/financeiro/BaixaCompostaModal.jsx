      ...
      <section className="flex max-h-[94vh] w-full max-w-7xl flex-col overflow-hidden rounded-2xl border border-[var(--modal-border)] bg-[var(--modal-bg)] shadow-2xl">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          <div className="mb-4 grid gap-3 md:grid-cols-[220px_1fr_auto]">
            ...
            <div className="self-end rounded-xl border border-[var(--c-border)] px-4 py-2"><small className="block text-[var(--c-muted)]">Saldo selecionado</small><strong>{money(totalSaldo)}</strong></div>
          </div>
        ...
        <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--c-border)] px-5 py-4"><span className="text-sm text-[var(--c-muted)]">Fontes: <strong className="text-[var(--c-text)]">{money(totalComponents)}</strong></span><div className="flex gap-2"><button type="button" className="btn btn-outline" onClick={onClose}>Cancelar</button>{preview ? <button type="button" className="btn btn-primary" disabled={saving || !compatible} onClick={confirm}>Confirmar baixa atômica</button> : <button type="button" className="btn btn-primary" disabled={saving || !compatible} onClick={review}>Validar e revisar</button>}</div></footer>
      </section>
    </div>
  );
}
